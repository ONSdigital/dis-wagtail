...
The `InvalidateReplayRedisCache` cache backend supports "replaying" cache invalidations to a second cache (configured as `invalidate_replay`). Whenever caches are purged, they're first removed from the default backend, and then additionally replayed to the `invalidate_replay` backend. `clear` is not replayed to allow separate entire cache purges.
