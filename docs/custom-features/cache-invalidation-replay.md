...
The `InvalidateReplayRedisCache` cache backend supports "replaying" cache invalidations to a second cache (configured as `invalidate_replay`). Whenever caches are purged, they're first replayed to the `invalidate_replay` backend before being run against the `default`. `clear` is not replayed to allow separate entire cache purges.
