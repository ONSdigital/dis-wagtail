...
def send_team_added_to_bundle_in_review_email(bundle_team: BundleTeam) -> None:
    ...
    active_user_emails = [user.email for user in team.users.all() if user.is_active]  # type: ignore[attr-defined]
    send_mail(
...
def send_bundle_status_changed_to_in_review_email(bundle: Bundle) -> None:
    ...
    bundle_teams = bundle.teams.get_object_list()  # type: ignore[attr-defined]
...
def send_bundle_published_email(bundle_team: BundleTeam) -> None:
    ...
    active_user_emails = [
        user.email
        for user in bundle_team.team.users.all()  # type: ignore[attr-defined]
        if user.is_active
    ]
    send_mail(
