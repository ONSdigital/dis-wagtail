...
def get_release_calendar_page_title_with_status_and_release_date(
    ...
    date = getattr(release_calendar_page.specific_deferred, "release_date_value", None)
    live_status = getattr(release_calendar_page.specific_deferred, "live_status", None)
    if live_status and date:
        return f"{title} ({live_status}) ({date})"
    if date:
        return f"{title} {date}"
    return f"{title}"
