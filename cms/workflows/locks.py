...
class PageWorkflowLock(WorkflowLock):
    ...
    When active, the page cannot be edited. Users can use "Unlock editing" to make changes.
    If the page is in a bundle that is ready to be published, the bundle must be reverted first.
    """
    ...
    def get_generic_message(self, user: User) -> str | SafeString:
        if self.for_user(user):
            return format_html(
                'This page cannot be edited as it is <strong>{status}</strong>. Use "Unlock editing" to make changes.',
