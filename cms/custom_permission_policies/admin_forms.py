...
class BaseMediaChildDocumentForm(WagtailBaseDocumentForm):
    """Activated via the documented WAGTAILDOCS_DOCUMENT_FORM_BASE setting, this base
    form ensures that the custom fields introduced by AbstractMediaChildDocument are
...
class BaseMediaChildImageForm(WagtailBaseImageForm):
    """Activated via the documented WAGTAILIMAGES_IMAGE_FORM_BASE setting, this base
    form ensures that the custom fields introduced by AbstractMediaChildImage are
