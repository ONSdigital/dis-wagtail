...
@step("the user adds a Definition snippet")
def user_adds_definition_snippet(context: Context) -> None:
    user_fills_in_definition_details(context)
    context.page.get_by_role("button", name="Save").click()


@given("the user fills in Definition details")
def user_fills_in_definition_details(context: Context) -> None:
    context.page.get_by_role("link", name="Snippets").click()
    context.page.get_by_role("link", name="Definitions").click()
    context.page.get_by_role("link", name="Add definition").click()
...
def validation_error_is_displayed_for_duplicated_definition(
    context: Context,
) -> None:
    expect(context.page.get_by_text("The definition could not be created due to errors.")).to_be_visible()
...
def user_edits_definition(context: Context) -> None:
...
def the_definition_is_visible_in_preview_tab(context: Context) -> None:
...
@then("the user can click the Definition to see the definition in the preview tab")
def the_definition_content_is_visible_in_preview_tab(context: Context) -> None:
    iframe_locator = context.page.frame_locator("#w-preview-iframe")
...
def the_user_can_see_the_preview_of_the_definition(context: Context) -> None:
...
@then("the user can click the Definition to see the definition")
def the_user_clicks_the_definition_to_see_the_definition(context: Context) -> None:
